...
public class H2SubjectDAOImplTest extends BaseTest {
  ...
  public H2SubjectDAOImplTest(String name) throws Exception {
    ...
    Person p = DAOFactory.getFactory().getPersonDAO().getById(2);
    Semester s = DAOFactory.getFactory().getSemesterDAO().getById(3);
  ...
  public void testShouldGetSubjectById() throws Exception {
    ...
    Assert.assertEquals("Mismatch in phone number", sub.getTeacher().getId(), expectedTable.getValue(0, "teacher_id"));
    Assert.assertEquals("Mismatch in phone number", sub.getSemester().getId(), expectedTable.getValue(0, "semester_id"));
  }
